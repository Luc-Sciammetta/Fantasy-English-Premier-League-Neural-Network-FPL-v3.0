...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_without_improvement = 0
            torch.save(model.state_dict(), "predictFuturePoints/misc/best_model.pth")
        else:
    ...
    #test the model
    model.load_state_dict(torch.load('predictFuturePoints/misc/best_model.pth'))
    torch.save(model.state_dict(), "predictFuturePoints/misc/fpl_model.pth")
    ...
    torch.save(model.state_dict(), f'predictFuturePoints/{mae:.4f}_best_model.pth')
    joblib.dump(scaler, f'predictFuturePoints/{mae:.4f}_scaler.pkl')  #save the scalers for later use
