...
def cleanPlayerDataframe(df, season):
    ...
    assists_per_90_season_to_date = [] #the player's number of assists per 90 minutes in the season to date (up to the current gw)
    xAssists_per_90_season_to_date = [] #the player's number of xAssists per 90 minutes in the season to date (up to the current gw)
    ...
    for index, row in df.iterrows(): #for each gameweek for the player
        ...
        assists_scored_season_to_date = df.iloc[:index]['assists'].sum()
        xAssists_season_to_date = df.iloc[:index]['expected_assists'].sum()

        assists_per_90_season_to_date.append((assists_scored_season_to_date / minutes_played_season_to_date * 90) if minutes_played_season_to_date > 0 else np.nan)
        xAssists_per_90_season_to_date.append((xAssists_season_to_date / minutes_played_season_to_date * 90) if minutes_played_season_to_date > 0 else np.nan)
...
def getFullDataset():
    ...
    final_df.to_csv('predictAssists/assists_training_data.csv', index=False)
