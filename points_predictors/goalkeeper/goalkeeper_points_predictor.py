...
def main():
    data = getDataFromCSV('get_historical_data/goalkeeper/goalkeeper_data.csv')
    ...
    val_dataset = GoalkeeperDataset(x_val_tensor, y_val_tensor)
    test_dataset = GoalkeeperDataset(x_test_tensor, y_test_tensor)
    ...
    model = GoalkeeperModel(input_size=x_train_tensor.shape[1])
    ...
    torch.save(model.state_dict(), f'points_predictors/goalkeeper/models/{mae6:.4f}_goalkeeper.pth')
    joblib.dump(scaler, f'points_predictors/goalkeeper/models/{mae6:.4f}_goalkeeper_scaler.pkl')  #save the scalers for later use
