...
def main():
    data = getDataFromCSV('get_historical_data/defender/defender_data.csv')
    ...
    train_dataset = DefenderDataset(x_train_tensor, y_train_tensor)
    val_dataset = DefenderDataset(x_val_tensor, y_val_tensor)
    test_dataset = DefenderDataset(x_test_tensor, y_test_tensor)
    ...
    model = DefenderModel(input_size=x_train_tensor.shape[1])
    ...
    torch.save(model.state_dict(), f'points_predictors/defender/models/{mae6:.4f}_defender.pth')
    joblib.dump(scaler, f'points_predictors/defender/models/{mae6:.4f}_defender_scaler.pkl')  #save the scalers for later use
